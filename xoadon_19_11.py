...
def row_is_highlighted(row):
    """
    Trả về True nếu dòng đã được tô màu (có fill khác mặc định).
    Mục đích: chỉ duyệt những dòng CHƯA tô màu.
    """
    for cell in row:
        fill = cell.fill
        # fill_type None hoặc "none" là chưa tô
...
# ====== HÀM MỚI: RETRY POPUP ĐỒNG Ý/OK KHI BỊ CHE BỞI JCONFIRM ======
def retry_delete_confirm_if_jconfirm(driver, wait, logger: UILogger = None):
    """
    Nếu còn popup jConfirm của thao tác XÓA đơn đang che màn hình,
    thì thực hiện lại chuỗi:
      - Nhấn ĐỒNG Ý (nút cam)
      - Chờ xử lý AJAX
      - Nhấn OK (popup thông báo)
      - Đợi tất cả popup đóng

    Trả về True nếu đã xử lý được (hoặc không còn popup cần xử lý),
    False nếu thất bại.
    """
    try:
        # Kiểm tra còn popup jConfirm nào hiển thị không
        ...
        if not has_open:
            # Không có popup jConfirm nào che thì thôi
            return False
        ...
        # Chờ AJAX xử lý và popup đóng bớt
...
def chon_ban_ghi_dau_tien(driver, timeout=30, logger: UILogger = None):
    ...
    try:
        ...
        # Bị overlay jConfirm che, đúng case bạn gặp
        if "jconfirm-scrollpane" in str(e):
            ...
            if not handled:
                # Không xử lý được popup -> ném lỗi ra ngoài
                raise

            # Sau khi xử lý lại popup, thử chọn lại bản ghi lần 2
            if logger:
            ...
            # Các trường hợp click intercepted khác thì cứ để lỗi bung ra
            raise
        ...
        def runner():
            ...
            try:
                ...
                for row_idx, row in enumerate(sheet.iter_rows(min_row=2), start=2):
                    # 🔴 Nếu dòng đã tô màu -> bỏ qua, không xử lý nữa
                ...
                logger.log(f"📄 File kết quả: {result_path}")

                # --- Chuẩn bị workbook KẾT QUẢ ---
                next_stt = 1  # STT sẽ dùng khi ghi

                if os.path.exists(result_path):
                    # Nếu file đã tồn tại -> mở lên và ghi nối tiếp
                    logger.log("📄 File kết quả đã tồn tại, sẽ ghi nối tiếp vào cuối file.")
                    result_wb = openpyxl.load_workbook(result_path)
                    result_ws = result_wb.active

                    last_row = result_ws.max_row
                    last_stt = result_ws.cell(row=last_row, column=1).value  # cột STT

                    if isinstance(last_stt, int):
                        next_stt = last_stt + 1
                    else:
                        # Trường hợp gì đó lạ (vd: chỉ có header), thì coi như bắt đầu từ (số dòng hiện tại)
                        next_stt = last_row
                else:
                    # Nếu chưa có file -> tạo mới và ghi header
                    ...
                    next_stt += 1
                    logger.log("📄 Chưa có file kết quả, tạo file mới.")
                ...
                for i, (row_num, so_to, so_thua) in enumerate(plots_to_process):
                    ...
                    # Chỉ tô màu nếu không phải lỗi (note không bắt đầu bằng 'Lỗi')
                    ...
                    # Lưu file gốc + kết quả mỗi 50 dòng
                    if (i + 1) % 50 == 0:
                        try:
